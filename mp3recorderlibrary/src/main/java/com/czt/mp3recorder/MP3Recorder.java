...
public class MP3Recorder {
	...
	public int getVoiceLevel(int maxlevel) {
		if (mPCMBuffer != null && mPCMBuffer.length != 0) {
			try {
				...
				for (int i = 0; i < mPCMBuffer.length; i++) {
					maxAmplitude += mPCMBuffer[i] * mPCMBuffer[i];
				}
				// 平方和除以数据总长度，得到音量大小。
				maxAmplitude = maxAmplitude / mPCMBuffer.length;
				System.out.println("maxAmplitude:" + maxAmplitude);
				// 2700-3000
				// 755603-755990
				if (maxAmplitude < 755603) {
					voiceLevel = 1;
				} else {
					voiceLevel = (int) ((maxAmplitude - 755603) / 300 * 7) + 1;
					System.out.println("voiceLevel:" + voiceLevel);
				}
				if (voiceLevel > maxlevel) {
					voiceLevel = maxlevel;
				}
				return voiceLevel;
			} catch (Exception e) {
